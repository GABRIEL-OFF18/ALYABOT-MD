...
let handler = async (m, { conn }) => {
  ...
  try {
    ...
    await conn.sendMessage(m.chat, {
      image: buffer,
      caption: `¡Ping completado! ⚡ Latencia: ${latencia.toFixed(2)} ms`,
      mimetype: 'image/png'
    }, { quoted: m })

  } catch (err) {
    console.error('Error generando imagen ping:', err.message || err)
    // Fallback texto bonito si canvas falla
    const fallback = `
*⚡ BALDUINO-MD STATUS ⚡*
...
¡Bot listo! 🚀`
    await conn.reply(m.chat, fallback, m)
  }
}
